...
func buildAPICoreServices(runtimeCfg config.Runtime, repo *chrepo.Repo, calendarRepo *calendarrepo.Repo, universeRepo *universerepo.Repo, logoRepo *logorepo.Repo, cacheStore cache.Store, factorStore *feeds.Store) (*apiCoreServices, error) {
	...
	universeSvc := service.NewUniverseService(repo, universeRepo).
		...
		WithETFClassifier(companyProfileProvider)
...
func buildAPIDeps(runtimeCfg config.Runtime, repo *chrepo.Repo, factorStore *feeds.Store, services *apiCoreServices, polygonSvc *service.PolygonService, deribitSvc *service.DeribitService, cacheStore cache.Store, apiKeyAuth api.APIKeyAuthenticator, stop chan struct{}) api.Deps {
	return api.Deps{
		Config:            runtimeCfg,
