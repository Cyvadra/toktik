...
func writeBacktestTutorial(builder *strings.Builder) {
	...
	builder.WriteString("- `request.security(...)`、`request.factor(...)`、`request.fundamental(...)` 的 market/symbol/interval/field 參數需使用字面量，validate 階段才能預先註冊依賴；動態 `request.security(\"us\", portfolio.symbol(0), ...)` 不會自動載入額外資料。\n")
