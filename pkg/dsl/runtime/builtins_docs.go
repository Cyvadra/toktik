...
var builtinDocOverrides = map[string]BuiltinDoc{
	...
	"options.strategies":          {Params: []string{"context", "family"}, Summary: "用於依市場環境分類挑選候選期權策略名稱。", Example: "names = options.strategies(ctx, \"momentum\")", ReturnValue: "陣列"},
	"options.build_strategy":      {Params: []string{"chain", "name", "qty", "target_delta"}, Summary: "用於把策略名稱和期權鏈轉成可檢查的標準 legs，不直接下單。", Example: "legs = options.build_strategy(chain, \"BUY_CALL\", 1, 0.35)", ReturnValue: "leg 陣列"},
