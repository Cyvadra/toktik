...
var builtinDocOverrides = map[string]BuiltinDoc{
	...
	"snapshot":                        {Params: []string{"value"}, Summary: "取得 series 在目前 bar 的標量快照；適合把 bar_index 或動態屬性保存到 var/varip，而不保留 live Series 引用。", Example: "entry_bar := snapshot(bar_index)", ReturnValue: "目前值"},
	"array.contains":                  {Params: []string{"items", "value"}, Summary: "檢查陣列是否包含指定值；可用於比對 point-in-time universe 與目前持倉標的。", Example: "still_member = array.contains(symbols, contract.underlying(leg))", ReturnValue: "布林"},
	"array.percentile":                {Params: []string{"items", "percentile"}, Summary: "忽略缺失值後，以線性插值計算數值陣列的百分位數；percentile 範圍為 0 到 100。", Example: "median = array.percentile(values, 50)", ReturnValue: "數值"},
	...
	"strategy.entry":                  {Params: []string{"id", "direction", "qty", "limit", "stop", "twap_bars", "immediate", "note", "notional"}, Summary: "用於以命名 ID 建立多頭或空頭部位；qty 與 notional 二選一，notional 大於零時按名義金額延遲計算數量。", Example: "strategy.entry(id=\"long\", direction=strategy.long, notional=10000)", ReturnValue: "na"},
	"strategy.close":                  {Summary: "嚴格依 entry ID 提交 reduce-only 平倉；未知 ID 不會影響其他倉位，同一 ID 已有 pending close 時不重複提交。", Example: "strategy.close(id=\"long\")", ReturnValue: "na"},
	"strategy.exit":                   {Summary: "strategy.close 的相容別名，同樣嚴格依 entry ID 執行 reduce-only 平倉。", Example: "strategy.exit(id=\"long\")", ReturnValue: "na"},
	...
	"options.expirations":             {Params: []string{"chain"}, Summary: "用於取得期權鏈中去重並按時間升序排列的 UTC Unix 秒到期日。", Example: "expirations = options.expirations(chain)", ReturnValue: "陣列"},
	"options.expiry":                  {Params: []string{"chain", "expiration"}, Summary: "用於保留 UTC Unix 秒到期日精確相符的合約。", Example: "expiry_chain = options.expiry(chain, expirations[0])", ReturnValue: "期權鏈"},
	"options.min_iv":                  {Params: []string{"chain"}, Summary: "用於取得鏈中 IV 最低的有效合約；忽略非正數與缺失 IV。", Example: "contract = options.min_iv(expiry_chain)", ReturnValue: "期權合約"},
	"options.lowest_iv":               {Params: []string{"chain", "n"}, Summary: "用於取得鏈中 IV 最低的前 n 個有效合約，由低到高排序；忽略非正數與缺失 IV。", Example: "contracts = options.lowest_iv(expiry_chain, 5)", ReturnValue: "陣列"},
	...
	"options.iv_smile_surface":        {Params: []string{"chain", "max_strike_distance_ratio"}, Summary: "用於從目前 bar 的期權鏈建立所有到期日的 OI 加權 IV smile surface；不會自動選擇到期日。", Example: "surface = options.iv_smile_surface(options.chain(\"crypto-options\", \"BTC\"), 0.20)", ReturnValue: "IV smile surface"},
	"options.iv_smile_expirations":    {Params: []string{"surface"}, Summary: "用於取得 IV smile surface 中所有到期日的 UTC Unix 秒，供策略顯式選擇期限。", Example: "expiries = options.iv_smile_expirations(surface)", ReturnValue: "陣列"},
	"options.iv_smile":                {Params: []string{"surface", "expiration"}, Summary: "用於從 IV smile surface 精確取得指定 UTC Unix 秒到期日的 Call/Put smile。", Example: "smile = options.iv_smile(surface, expiries[0])", ReturnValue: "IV smile"},
	"options.iv_smile_expiry":         {Params: []string{"smile"}, Summary: "用於取得 IV smile 的 UTC Unix 秒到期日。", Example: "expiry = options.iv_smile_expiry(smile)", ReturnValue: "數值"},
	"options.iv_smile_total_oi":       {Params: []string{"smile"}, Summary: "用於取得指定期限 Call 與 Put 曲線的總 OI。", Example: "oi = options.iv_smile_total_oi(smile)", ReturnValue: "數值"},
	"options.iv_smile_oi_coverage":    {Params: []string{"smile", "option_type"}, Summary: "用於取得指定 Call 或 Put 曲線中 OI 大於零的點占比。", Example: "coverage = options.iv_smile_oi_coverage(smile, \"put\")", ReturnValue: "數值"},
	"options.iv_smile_strikes":        {Params: []string{"smile", "option_type"}, Summary: "用於取得指定 Call 或 Put IV smile 的 strike 陣列。", Example: "strikes = options.iv_smile_strikes(smile, \"call\")", ReturnValue: "陣列"},
	"options.iv_smile_values":         {Params: []string{"smile", "option_type", "smoothed"}, Summary: "用於取得 raw 或 OI 加權平滑後的 IV 陣列；預設回傳平滑 IV。", Example: "ivs = options.iv_smile_values(smile, \"put\", true)", ReturnValue: "陣列"},
	"options.iv_smile_open_interests": {Params: []string{"smile", "option_type"}, Summary: "用於取得與 IV smile strike 對齊的 OI 陣列。", Example: "ois = options.iv_smile_open_interests(smile, \"call\")", ReturnValue: "陣列"},
	"options.iv_smile_at":             {Params: []string{"smile", "option_type", "strike", "smoothed"}, Summary: "用於取得指定 strike 的 IV；曲線內部採線性插值，範圍外回傳 na。", Example: "put_iv = options.iv_smile_at(smile, \"put\", close, true)", ReturnValue: "數值"},
...
var builtinExampleByName = map[string]string{
	...
	"options.expirations":    "expirations = options.expirations(options.chain(\"us-options\", \"SPY\"))",
	"options.expiry":         "chain = options.expiry(chain, expirations[0])",
	"options.min_iv":         "contract = options.min_iv(chain)",
...
var builtinParamHints = map[string][]string{
	...
	"array.contains":                  {"items", "value"},
	"snapshot":                        {"value"},
	"array.percentile":                {"items", "percentile"},
	...
	"options.iv_smile_expiry":         {"smile"},
	"options.iv_smile_total_oi":       {"smile"},
	"options.iv_smile_oi_coverage":    {"smile", "option_type"},
	"options.iv_smile_strikes":        {"smile", "option_type"},
	"options.iv_smile_values":         {"smile", "option_type", "smoothed"},
	"options.iv_smile_open_interests": {"smile", "option_type"},
	"options.iv_smile_at":             {"smile", "option_type", "strike", "smoothed"},
