...
- 12H HV <= percentile(HV_12h, 150, 66) ； HV <= 150根12h HV的 n 分位数
- 12H DVOL <= percentile(IV_12h, 150, 66) ； DVOL <= 150根12h IV的 n 分位数
- 12H K线数量不足 200 根时，判断 12H DVOL/HV <= 96

初始资金： 100 BTC

仅交易spreads

备注：策略应运行在 12H 主周期上，HV/IV 的统计窗口按 12H K 线定义。
...
- A => 当前 HV，相当于 12h HV 历史 150根 的 A 分位 （0-100）
