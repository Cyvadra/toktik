...
首仓额外过滤条件： 收到 init 开仓信号后，仅要求当前 12H DVOL 的 B 分位不大于 96，否则忽略该次首仓信号。
根据 12H 的历史数据，计算 `vol_std` 和 IV，相关分位统计窗口统一使用 200 根 12H K 线。
...
首仓 amount_base = 100 BTC * 3% = 3.0 BTC；该基数固定按初始总资产计算，不复利。
...
- A => 当前 `vol_std`，相当于 12H `vol_std` 历史 200 根的 A 分位（0-100）

- B => 当前 DVOL，相当于 12h IV 历史 200 根的 B 分位
...
备注：首仓固定使用基于初始总资产 3% 计算得到的 3 BTC；后续换仓仅按 9 折递减，不做复利扩张。
...
- (price2buy - price2sell) 相对于开仓时，上涨50% （按mark计），且 abs(delta_l) >= 0.45

- 订单组中的多头腿A 实时 delta 相对开仓时增加 0.2，且 abs(delta_l) >= 0.45

### 换仓系数

订单组每次换仓，投入金额衰减，按上次投入的90%，进行换仓的开仓；

例如 首仓3BTC ==> 换仓2.7BTC ==> 再次换仓时2.43BTC
