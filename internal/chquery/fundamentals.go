...
func FundamentalSeriesAsOfQuery() string {
	return fmt.Sprintf(`SELECT
	event_ts,
	argMax(known_at, (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0))) AS latest_known_at,
	argMax(value,    (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0))) AS latest_value,
	argMax(source,   (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0))) AS latest_source
FROM %s
WHERE market = {market:String}
  AND symbol = {symbol:String}
  AND factor_code = {factor:String}
...
func FundamentalSnapshotQuery() string {
	return fmt.Sprintf(`SELECT
	factor_code,
	argMax(event_ts, (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0)))  AS latest_event_ts,
	argMax(known_at, (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0)))  AS latest_known_at,
	argMax(value,    (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0)))  AS latest_value,
	argMax(source,   (known_at, revision, if(source = 'fmp_statement_derived_v2', 1, 0)))  AS latest_source
FROM %s
